...
function [texStr,stimData] = NewsomeDots(Q)
...
% Each dot gets a row. The first column indexes the X axis, second row, Y
% axis. The third dimension separates each population into two to keep
% mean lum consistent. The fourth dimension holds the last 2*deltaT dot
% matrices for retrieval in subsequent calls. 

if fr == 1
    stimData.mat=zeros(population,3,2,deltaT*2);
end

%% update dots

dots = zeros(population,3,2); 
...
if fr ==1
    for ii = 1:deltaT
        ...
        dots(:,3,2) = increment * (Hi+mLum) + ( 1 - increment ) * (Lo+mLum); 
...
for k=linspace(1,framesPerUp,framesPerUp);
    ...
    dots(:,3,1) = increment * (Lo+mLum) + ( 1 - increment ) * (Hi+mLum); % set contrast
    ...
    dots(:,3,2) = increment * (Hi+mLum) + ( 1 - increment ) * (Lo+mLum); % set contrast
    ...
    dots(:,3,1) = increment * (Hi+mLum) + ( 1 - increment ) * (Lo+mLum);
    % initialize second subpopulation
    dots(:,1,2) = ceil(rand(population,1,1)*btmpWd);
    dots(:,2,2) = ceil(rand(population,1,1)*btmpHt);
    dots(:,3,2) = increment * (Lo+mLum) + ( 1 - increment ) * (Hi+mLum);
    ...
    dots(:,3,1) = increment * (Lo+mLum) + ( 1 - increment ) * (Hi+mLum);
    % second subpopulation is the shifted/contrast changed version of this
    % one, but they must be presented simultaneously. Does thie impact
    % density?
