function [texStr,stimData] = GliderAnyPoint(Q)
...
if f == 1
    if ~isfield(stimData,'glidMemL') || (sizeY ~= size(stimData.glidMemL,1)) || (sizeX ~= size(stimData.glidMemL,2))
...
for cc = 1:framesPerUp
    if updateFrameL(cc)
        ...
        if (sizeY ~= size(stimData.glidMemL,1)) || (sizeX ~= size(stimData.glidMemL,2))
            stimData.glidMemL = 2*(round(rand(sizeY,sizeX,maxDelayL)))-1;
        end
        ...
        if delayL == 0
            ...
            switch corrTypeL
                ...
                case 5
                    ...
                    for xx = 2:sizeX
                        for yy = 1:sizeY
                            newMatL(yy,xx) = corrSignL/(oldMatL(yy-1,xx)*oldMatL(yy,xx)*newMatL(yy-1,xx));
                        end
                    end
                    ...
                    case 10
                        ...
                        oldMatL = stimData.glidMemL(:,:,delayL);
                        newMatL(:,1) = floor(3*rand(sizeY,1,1))-1;

                        for xx = 2:sizeX
                            for yy = 1:sizeY
                                if ~isinf(1/(oldMatL(yy,xx-1)*oldMatL(yy,xx)*newMatL(yy,xx-1)))
                                    newMatL(yy,xx) = 0;
                                else
                                    newMatL(yy,xx) = floor(3*rand)-1;
                                end
                            end
                        end
            end
...
if p.twoEyes
    ...
    for cc = 1:framesPerUp
        if updateFrameR(cc)
            ...
            if (sizeY ~= size(stimData.glidMemR,1)) || (sizeX ~= size(stimData.glidMemR,2))
                stimData.glidMemR = 2*(round(rand(sizeY,sizeX,maxDelayR)))-1;
            end
            ...
            if delayR == 0
                ...
                switch corrTypeR
                    case 2
                        newMatR = corrSignR*circshift(stimData.glidMemR(:,:,delayR),[dirYR,dirXR]);
                    ...
                    case 5
                        ...
                        for xx = 2:sizeX
                            for yy = 1:sizeY
                                newMatR(yy,xx) = corrSignR/(oldMatR(yy-1,xx)*oldMatR(yy,xx)*newMatR(yy-1,xx));
                            end
                        end
                        ...
                        case 10
                            ...
                            oldMatR = stimData.glidMemR(:,:,delayR);
                            newMatR(:,1) = floor(3*rand(sizeY,1,1))-1;

                            for xx = 2:sizeX
                                for yy = 1:sizeY
                                    if ~isinf(1/(oldMatR(yy,xx-1)*oldMatR(yy,xx)*newMatR(yy,xx-1)))
                                        newMatR(yy,xx) = 0;
                                    else
                                        newMatR(yy,xx) = floor(3*rand)-1;
                                    end
                                end
                            end
                end
