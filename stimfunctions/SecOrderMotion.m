function [texStr,stimData] = SecOrderMotion(Q)
    ...
    vel2L = p.velocity2L*pi/180;
    vel2R = p.velocity2R*pi/180;

    lambdaL = p.lambdaL*pi/180; %wavelength in radians
    lambdaR = p.lambdaR*pi/180; %wavelength in radians

    lambda2L = p.lambda2L*pi/180; %wavelength in radians
    lambda2R = p.lambda2R*pi/180; %wavelength in radians
    
    velSTDL = p.velSTDL*pi/180;
    velSTDR = p.velSTDR*pi/180;

    velSTD2L = p.velSTD2L*pi/180;
    velSTD2R = p.velSTD2R*pi/180;
    ...
    if f == 1
        ...
        if ~isfield(stimData,'sinPVL')
            stimData.sinPVL = zeros(4,1);
        end

        if ~isfield(stimData,'sinPVR')
            stimData.sinPVR = zeros(4,1);
        end
    ...
    for cc = 1:framesPerUp
        if upVelL(cc) == 1
            ...
            stimData.sinPVL(4) = randn*velSTD2L+vel2L;
        end

        if upPosL(cc) == 1
            stimData.sinPVL(1) = stimData.sinPVL(1) + stimUpRateL*stimData.sinPVL(2)/(60*framesPerUp);
            stimData.sinPVL(3) = stimData.sinPVL(3) + stimUpRateL*stimData.sinPVL(4)/(60*framesPerUp);
        end

        bitMap(1,:,cc) = cL*sin(2*pi*(theta-stimData.sinPVL(1))/lambdaL).*sin(2*pi*(theta-stimData.sinPVL(3))/lambda2L);
...
%        stimData.mat(4*cc-1:4*cc) = stimData.sinPVL;
    end
