function [texStr,stimData] = SineWaveMirrored(Q)
    ...
    if f == 1
        stimData.sinPhase = 0;
        thetaOrig = (0:sizeX-1)/sizeX*2*pi; %theta in radians
        thetaMat = reshape(thetaOrig,chunkSize,[]);
        numChunks = size(thetaMat,2);
        thetaMatRand = thetaMat(:,randperm(numChunks));
        flipChunks = rand(numChunks,1) > 0.5;
        thetaMatFlip = thetaMatRand;
        thetaMatFlip(:,flipChunks) = flipud(thetaMatFlip(:,flipChunks));
        stimData.scrambledTheta = thetaMatFlip(:);
    end
    ...
    for cc = 1:framesPerUp
        ...
        bitMap(1,:,cc) = c*sin(2*pi*(stimData.scrambledTheta-stimData.sinPhase)/lambda);
