function [texStr,stimData] = ScintillatorBinarizedMirrored(Q)
    ...
    if f == 1
        ...
        if ~isfield(stimData,'scintA')
            ...
            for ii = 1:size(Q.stims.params,2)
                ...
                if Q.stims.params(ii).delay >= stimData.maxDelay
                    stimData.maxDelay = Q.stims.params(ii).delay + 1;
                end
        
                checkForTwoEyes = checkForTwoEyes + Q.stims.params(ii).twoEyes;
            end
            
            stimData.scintA = randn(sizeY,sizeX,stimData.maxDelay);
        end

        if ~isfield(stimData,'scintB');
            stimData.scintB = randn(sizeY,sizeX);
        end
    end
    ...
    updateNoise = mod(hzFrame-1,noiseUp) == 0;
    ...
    for cc = 1:framesPerUp
        if updateFrame(cc)
            ...
            if sizeY == size(stimData.scintA,1) && sizeX == size(stimData.scintA,2)
                stimData.scintA = cat(3,randn(sizeY,sizeX),stimData.scintA(:,:,1:end-1));
            else
                stimData.scintA = randn(sizeY,sizeX,stimData.maxDelay);
            end
        end
        ...
        bitMap(:,:,cc) = A*stimData.scintA(:,:,1)+phi*A*circshift(stimData.scintA(:,:,delay+1),[dirY,dirX])+B*stimData.scintB;
    end
