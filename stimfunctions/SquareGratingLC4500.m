function [texStr,stimData] = SquareGratingLC4500(Q)
%     tic;
...
%     tic;
    ...
    if ~isfield(params,'numSamples')
        params.numSamples = 128;
    end
    
    if strcmp(params.orientation,'x')
        bitMap(1,params.numSamples,12) = 0;
        theta = linspace(0,2*pi,params.numSamples);
    else
        bitMap(params.numSamples,1,12) = 0;
        theta = linspace(0,2*pi,params.numSamples)';
    end
...
%     tic
    ...
    for f = 1:12
        if selectedColorFrames(f)
            ...
            bitMap(:,:,f) = square((theta-phase)*2*pi/wavelengthRadians);
