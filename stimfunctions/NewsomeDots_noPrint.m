function [texStr,stimData] = NewsomeDots_noPrint(Q)
...
if fr == 1
    stimData.mem=zeros(population,4,2,deltaT*2);
end
...
for k=linspace(1,framesPerUp,framesPerUp);
    ...
    dots(:,1,2) = stimData.mem(:,1,2,deltaT) + deltaX; % varies from 1-deltaX to btmpWd+deltaX
    % the tricky thing is whether I'm using stimData.mem right here
    dots(:,2,2) = stimData.mem(:,2,2,deltaT) + deltaY; % varies from 1-deltaY to btmpWd+deltaY
    dots(:,3,2) = increment .* (Hi) + ... 
        ( 1 - increment ) .* (Lo); % set contrast
    dots(:,4,2) = stimData.mem(:,4,1,deltaT);
    
    elseif ~first && ~simul

    % shift first subpopulation
    dots(:,1,1) = stimData.mem(:,1,1,deltaT) + deltaX; % call stimData.mem at deltaT because it hasn't been "rolled" yet
    dots(:,2,1) = stimData.mem(:,2,1,deltaT) + deltaY;
    dots(:,3,1) = increment .* (Hi) + ... 
        ( 1 - increment ) .* (Lo); % set contrast
    dots(:,4,1) = stimData.mem(:,4,1,deltaT);   
    ...
    stimData.mem(:,:,:,2:end) = stimData.mem(:,:,:,1:end-1);
    stimData.mem(:,:,:,1) = dots;
    ...
    for z = 1:population % go through all the dots (tot # = population)
        for a = 1:2 % born v. kill subpops
            x = dots(z,1,a);
            y = dots(z,2,a);
            preMap(y+abs(deltaY),x+abs(deltaX)) = dots(z,4,a)*dots(z,3,a)+mLum;
