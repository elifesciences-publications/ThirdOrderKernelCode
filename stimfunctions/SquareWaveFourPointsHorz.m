function [texStr,stimData] = SquareWaveFourPointsHorz(Q)
    ...
    if f == 1
        ...
        if ~isfield(stimData,'sin4pPhaseL') || ~p.sameStructL
            stimData.sin4pPhaseL = floor(4*rand(sizeY,1))+1;
        end
        
        if ~isfield(stimData,'sin4pPhaseR') || ~p.sameStructR
            stimData.sin4pPhaseR = floor(4*rand(sizeY,1))+1;
        end
    end
    ...
    if p.twoEyes
        ...
        rightEye = repmat(rightEye,[1,sizeX]);
