function [texStr,stimData] = GliderMirrored(Q)
...
if f == 1
    if ~isfield(stimData,'glidMem') || (sizeY ~= size(stimData.glidMem,1)) || (sizeX ~= size(stimData.glidMem,2))
...
updateFrameL = mod(hzFrame-1,update) == 0;
...
for cc = 1:framesPerUp
    if updateFrameL(cc)
        ...
        if delay == 0
            ...
            switch corrType
                case 2
                    newMat = phi*circshift(stimData.glidMem(:,:,delay),[dirY,dirX]);
                ...
                case 4
                    ...
                    for xx = 2:sizeX
                        for yy = 1:sizeY
                            newMat(yy,xx) = phi/(oldMat(yy,xx-1)*oldMat(yy,xx)*newMat(yy,xx-1));
                        end
                    end
                ...
                case 5
                    ...
                    for xx = 2:sizeX
                        for yy = 1:sizeY
                            newMat(yy,xx) = phi/(oldMat(yy-1,xx)*oldMat(yy,xx)*newMat(yy-1,xx));
                        end
                    end
