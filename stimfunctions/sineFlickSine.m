function [texStr,stimData] = sineFlickSine(Q)
...
if f == 0
 ...
 stimData.age = 0;
%% Draw the bitmap
if f > p.duration/3 && f <= p.duration*2/3
if btmpWd == 1
...
%     bitMap(:,:,q) =  mLum * ( 1 + .95 * var * flickPlusSin);
    bitMap(:,:,q) =  mLum * ( 1 + (flickerVar * flickDec*stretchBars + sineVar*sinDec*stimData.sinWave));
...
for q = 1:framesPerFlip        
    ...
    % Put into bitmap
    bitMap(:,:,q) =  mLum * ( 1 + (sineVar*sinDec*stimData.sinWave));
    stimData.age = mod(stimData.age+1,lifespan); 
end
end
