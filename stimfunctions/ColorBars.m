function [texStr,stimData] = ColorBars(Q)
    ...
    startingPhaseProportion =  [params.phaseDegreesBlue params.phaseDegreesRed params.phaseDegreesGreen]/360;
    startingPhase = startingPhaseProportion.*wavelengthRadians;
    ...
    if ~isfield(stimData,'prevEpochLastPhase')
            stimData.prevEpochLastPhase = zeros(sizeY,sizeX,3); 
    end
    if ~isfield(stimData,'thisEpochLastPhase')
            stimData.thisEpochLastPhase = repmat(permute(startingPhase,[1 3 2]),sizeY,sizeX);
    end
    if updateNum == 1 %We are in a new epoch
        stimData.prevEpochLastPhase = stimData.thisEpochLastPhase;
        stimData.thisEpochLastPhase = zeros(sizeY,sizeX,3);
    end
    
    for f = 1:framesPerUpdate
        t =(updateNum-1)*(1/60);
        color = floor((f-1)/(framesPerUpdate/3))+1;
        phase = t*velocityRadians(color)*ones(sizeY,sizeX) + stimData.prevEpochLastPhase(:,:,color);
