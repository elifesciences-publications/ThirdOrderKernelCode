function [texStr,stimData] = flicker_sineCombined_v2(Q)
    ...
    %bitMap = zeros(72,72,framesPerUp);
...
%                 for k = 1:50
            ...
            % binary distribution
            %bitMap(:,:,i) = 2*p.stdv*(randi(2, [72 72])-1.5);
            bitMap(:,:,i) = 2*p.stdv*(randi(2)-1.5);
...
%                 end
    ...
    if f == p.duration/2+1
        ...
        if ~isfield(stimData,'sinWNL2');
            stimData.sinWNL2 = randn(sizeY,sizeX);
        end

        if ~isfield(stimData,'sinWNR2');
            stimData.sinWNR2 = randn(sizeY,sizeX);
        end
    end

    sinWNL2(sizeY,sizeX,framesPerUp) = 0;
...
%     bitMap = repmat(bitMap,[sizeY,1]);
    ...
    bitMap = mlumL*(1 + bitMap + wnCL*sinWNL2);
    ...
    if p.twoEyes
        ...
        %velR = -velR;
        sinWNR2(sizeY,sizeX,framesPerUp) = 0;
        ...
        rightEye = mlumR*(1 + rightEye + wnCR*sinWNR2);
