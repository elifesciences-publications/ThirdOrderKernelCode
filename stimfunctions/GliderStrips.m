function [texStr,stimData] = GliderStrips(Q)
...
if f == 0
    ...
    stimData.age = 0;
    stimData.map = 2*(randi(2,[maxDispT+1,maxDispX+2])-1.5);
...
for q = 1:framesPerUp   
    ...
    if diagDirec == -1
        bars = fliplr(stimData.map(1,end-1:end));
    else
        bars = stimData.map(1,end-1:end);
    end    
    
    core = [ bars grayContrast grayContrast ];
