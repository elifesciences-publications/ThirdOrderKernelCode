function [texStr,stimData] = SquareWave(Q)
...
if f == 1
    ...
    if ~isfield(stimData,'sinsinWNL');
        stimData.sinWNL = randn(sizeY,sizeX);
    end
    
    if ~isfield(stimData,'sinsinWNR');
...
bitMap = mlumL*(2*round(0.5 + bitMap/2) + wnCL*sinWNL);
...
if p.twoEyes
    ...
    rightEye = mlumR*(2*round(0.5 + rightEye/2) + wnCR*sinWNR);
