function [ OLS2,OLS3,XC2,XC3 ] = mixedFly_function( dur,whichOrder )
% 
tic
...
startDiag = 1; endDiag = 15; wingSpan = 5;
...
OLS2 = zeros(margin,margin);
OLS3 = zeros(margin,margin,margin);

if whichOrder(2)
    for rr = 1:seqInd(2)
        thisX = locs{2}.tau1(rr) + 1;
        thisY = locs{2}.tau2(rr) + 1;
        OLS2(thisX,thisY) = OLS_pols(rr);
    end
end
...
if whichOrder(3)
    for rr = 1:seqInd(3)
        ...
        OLS3(thisX1,thisX2,thisY) = OLS_pols(rr);
    end
end
...
XC3 = reshape(xCorr_kernel{3},[maxTau maxTau maxTau]);
XC2 = reshape(xCorr_kernel{2},[maxTau maxTau]);
