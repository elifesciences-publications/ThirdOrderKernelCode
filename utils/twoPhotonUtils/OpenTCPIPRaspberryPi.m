function connectionToRaspberryPi = OpenTCPIPRaspberryPi
...
raspberryPiIPAddress = '172.29.48.124';
...
connectionToRaspberryPi = tcpip(raspberryPiIPAddress, 30000, 'NetworkRole', 'client');
...
maxAttempts = 1;
while notConnected
    try fopen(connectionToRaspberryPi);
        notConnected = false;
    catch err
        ...
        if strcmp(err.identifier, 'instrument:fopen:opfailed')
            if timesAttempted < maxAttempts
                ...
                fprintf('Connection unsuccessful. Problems endemic. Try again once you''ve fixed them!')
                warning(err.message)
                connectionToRaspberryPi = [];
                break
            end
            pause(.2)
        else
