function MakeDirList(folders,savePath)
    ...
    if isempty(paths.folders{1})
        paths.folders = UiPickFiles('FilterSpec',dataFolder,'Prompt','Choose folders containing the files to be analyzed');
    end
    ...
    if ~savePath
        savePath = fullfile(dataFolder,'dataPath/',datestr(now,'yyyy/mm_dd/HH_MM_SS/'));
    end
