function backup_text_m_files(Q)
...
if(Q.toPunish)
    copyfile(Q.paths.chosenpunishparameterfile,Q.paths.data);
end
...
if(Q.toPunish)
    dpunishfunctions = dir(Q.paths.punishfunctions);
    dpunishparamfiles = dir(Q.paths.punishparamfiles);
end
...
if(Q.toPunish)
    ziplist{end+1} = Q.paths.punishfunctions;
    ziplist{end+1} = Q.paths.punishparamfiles;
end
...
if(Q.toPunish)
    fprintf(f,'***PUNISHPARAMFILES FILES\n');
    for ii = 1:length(dpunishparamfiles)
        if ~(dpunishparamfiles(ii).name(1)=='.')
            fprintf(f,'%s, %s\n',dpunishparamfiles(ii).name,dpunishparamfiles(ii).date);
        end
    end

    fprintf(f,'***PUNISHFUNCTIONS FILES\n');
    for ii = 1:length(dpunishfunctions)
        if ~(dpunishfunctions(ii).name(1)=='.')
            fprintf(f,'%s, %s\n',dpunishfunctions(ii).name,dpunishfunctions(ii).date);
        end
    end
end
f = fclose(f);
