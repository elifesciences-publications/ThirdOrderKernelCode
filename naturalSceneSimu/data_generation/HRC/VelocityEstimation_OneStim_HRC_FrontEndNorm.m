function v2 = VelocityEstimation_OneStim_HRC_FrontEndNorm(stim, stim_norm, kernel, varargin)
% space_range = 54;
...
for ii = 1:1:space_range - 1
    s1 = stim(:, ii);
    s2 = stim(:, ii + 1);
    s1_norm = stim_norm(:, ii);
    s2_norm = stim_norm(:, ii + 1);
    v2(ii) =  VelocityEstimation_Utils_HRC_FrontEndNorm(kernel, s1, s2, s1_norm, s2_norm);
end
