function DataTransferUniform(param)
...
vfilename = [path.data,vInfo.name,s];
% you are interested in all distribution, put them in different folder?


dateInfo = SearchFolder(vfilename);
ndate = length(dateInfo); 
nDataUnit = 0;
% first, know how many data are there for this particular velocity.
for tt = 1:1:ndate
    fDate = [vfilename,dateInfo(tt).name,s];
