function tp_plotGliderStimuli(Z, titleText, epochsForSelectivity, epochsOfInterestDir1, epochsOfInterestDir2)
...
[roiIndsOfInterestDir1, pValsSumDir1] = extractROIsBySelectivity(Z);
...
roi_image = [];
roi_data = [];
dataToPlotDir1 = Z.averageEpochResponseAnalysis;
epochsForPlottingDir1 = dataToPlotDir1.epochsOfInterest(epochsOfInterestDir1);


makeFigure
subplot(2, 2, 1)
if any(roiIndsOfInterestDir1)
    plotAverageEpochResponse(dataToPlotDir1.dFFEpochValues(:,:,epochsOfInterestDir1),params, roi_image, roi_data, epochsForPlottingDir1, [1 0 0]);
    dataMeanDir1 = nanmean(dataToPlotDir1.dFFEpochValues(:,:,epochsOfInterestDir2), 2);
else
    dataMeanDir1 = [];
end
...
if nargin < 5
    ...
    if ~isempty(dataMeanDir1)
        if ~isempty(dataMeanDir2)
            dataToPlotOverall = [dataToPlotDir1.dFFEpochValues(:,:,epochsOfInterestDir1); dataToPlotDir2.dFFEpochValues(:,:,epochsOfInterestDir2)];
        else
            dataToPlotOverall = [dataToPlotDir1.dFFEpochValues(:,:,epochsOfInterestDir1)];
        end
    elseif ~isempty(dataMeanDir2)
        dataToPlotOverall = [dataToPlotDir1.dFFEpochValues(:,:,epochsOfInterestDir2)];
    else
