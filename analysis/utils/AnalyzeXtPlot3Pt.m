function [listOfDxs, listOfDts]= AnalyzeXtPlot3Pt(dataPath,computeCorr,normSize)    
...
%     D.analysis.R = reichardtModel(xtPlot);
    ...
    if computeCorr3p
        for ee = 1:numEpochs
            epochXT.leftEye{ee} = thisEpoch{ee};
            [Dx1, Dx2, Dt1, Dt2] = ThreePtCorrFinder(epochXT.leftEye{ee}, ee);
            listOfDxs{ee} = [Dx1, Dx2];
            listOfDts{ee} = [Dt1, Dt2];
        end
    end
end