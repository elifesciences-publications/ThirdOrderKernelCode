function CombinedResp = CombineOppositeTrials(snipMat)
    ...
    for ff = 1:size(snipMat,2)
        % interlave doesn't change
        CombinedResp{1,ff} = snipMat{1,ff};
        
        for ee = 2:2:size(snipMat,1)
            [numTimePoints,numTrials1,~] = size(snipMat{ee  ,ff});
            [~            ,numTrials2,~] = size(snipMat{ee+1,ff});
            CombinedResp{ee/2+1,ff} = zeros(numTimePoints,numTrials1+numTrials2,2);

            CombinedResp{ee/2+1,ff}(:,:,1) = cat(2,snipMat{ee,ff}(:,:,1),-snipMat{ee+1,ff}(:,:,1));
            CombinedResp{ee/2+1,ff}(:,:,2) = cat(2,snipMat{ee,ff}(:,:,2), snipMat{ee+1,ff}(:,:,2));
        end
    end
end