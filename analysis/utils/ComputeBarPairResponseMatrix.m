function [out, outDesc, outSortMat] = ComputeBarPairResponseMatrix(roiAveragedResponses, barPairSortingStructure, barToCenter, mirrorCheck, optimalBar, numPhases, roiPhaseShift)
...
if mirrorCheck 
    ...
    % instead of being at the beginning of one of them
    phasesPerBar = sortingMatrix(8, sortingMatrix(2, :)==neighDir & sortingMatrix(4, :) ~= 0 & sortingMatrix(5, :) == 0)./sortingMatrix(10, sortingMatrix(2, :)==neighDir & sortingMatrix(4, :) ~= 0 & sortingMatrix(5, :) == 0);
    ...
    sortingMatrix(1, sortingMatrix(4, :)==0) = mod(sortingMatrix(1, sortingMatrix(4, :)==0)+phasesPerBar, numPhases);
    ...
    sortingMatrix(1, sortingMatrix(4, :)==0) = mod(sortingMatrix(1, sortingMatrix(4, :)==0)-phasesPerBar+1, numPhases);
end
...
PlusSingle = find(sortingMatrix(3, :)==1 & sortingMatrix(3, :).*sortingMatrix(4, :)==0 & sortingMatrix(12, :) == 1); % The normal plus single last 1 second
MinusSingle = find(sortingMatrix(3, :)==-1 & sortingMatrix(3, :).*sortingMatrix(4, :)==0 & sortingMatrix(12, :) == 1); % The normal plus single last 1 second

PlusHalfSingle = find(sortingMatrix(3, :)==1 & sortingMatrix(3, :).*sortingMatrix(4, :)==0 & sortingMatrix(12, :) == 0.5); % The half plus single lasts 0.5 seconds
MinusHalfSingle = find(sortingMatrix(3, :)==-1 & sortingMatrix(3, :).*sortingMatrix(4, :)==0 & sortingMatrix(12, :) == 0.5); % The half minus single lasts 0.5 seconds

PlusShortSingle = find(sortingMatrix(3, :)==1 & sortingMatrix(3, :).*sortingMatrix(4, :)==0 & sortingMatrix(12, :) == 0.15); % The short plus single lasts 0.15 seconds
MinusShortSingle = find(sortingMatrix(3, :)==-1 & sortingMatrix(3, :).*sortingMatrix(4, :)==0 & sortingMatrix(12, :) == 0.15); % The short minus single lasts 0.15 seconds
...
shift = 0;
% NOTE this optimal phase has to be well aligned iwth the
% PlotBarPairROISummary has so that the plotted responses can be aligned
% with the bar pair xt plots
optimalPhase = length(PlusSingle)/2; % This is the middle phase of the plot, assuming there are as many single phases as there are total phases
%optimalPhase = 10;
...
if isempty(roiPhaseShift)
    switch optimalBar
        case 'PlusSingle'
            singlePlusStillResp = cat(2, roiAveragedResponses{PlusSingleAlign, 1})';
        ...
        case 'MinusSingle'
            singleMinusStillResp = cat(2, roiAveragedResponses{MinusSingleAlign, 1})';
...
out = [pplusPrefResp; pplusNullResp; pminusPrefResp;   pminusNullResp;   nplusPrefResp; nplusNullResp; nminusPrefResp;  nminusNullResp; pplusDoubleResp; pminusDoubleResp; nplusDoubleResp; nminusDoubleResp; pplusNeighResp; pminusNeighResp; nplusNeighResp; nminusNeighResp; singlePlusHalfStillResp; singleMinusHalfStillResp; singlePlusShortStillResp; singleMinusShortStillResp; singlePlusStillResp; singleMinusStillResp];
outSortMat = sortingMatrix(:, [PPlusPref, PPlusNull, PMinusPref, PMinusNull, NPlusPref, NPlusNull, NMinusPref, NMinusNull, PPlusDouble, PMinusDouble, NPlusDouble, NMinusDouble, PPlusNeigh, PMinusNeigh, NPlusNeigh, NMinusNeigh, PlusHalfSingle, MinusHalfSingle, PlusShortSingle, MinusShortSingle, PlusSingle, MinusSingle]);
availableDescriptions = {'++ Pref Dir', '++ Null Dir', '-- Pref Dir', '-- Null Dir',...
    ...
    '+ Still', '- Still'}';
matsUsed = {pplusPrefResp; pplusNullResp; pminusPrefResp;   pminusNullResp;   nplusPrefResp; nplusNullResp; nminusPrefResp;  nminusNullResp; pplusDoubleResp; pminusDoubleResp; nplusDoubleResp; nminusDoubleResp; pplusNeighResp; pminusNeighResp; nplusNeighResp; nminusNeighResp; singlePlusHalfStillResp; singleMinusHalfStillResp; singlePlusShortStillResp; singleMinusShortStillResp; singlePlusStillResp; singleMinusStillResp};
