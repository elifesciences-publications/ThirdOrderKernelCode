function [figureHandles, axesHandles, barPairComboRespOut] = PlotBarPairROISummaryPointMotIncrease(roiSummaryMatrix, respSemOrCompMat, barToCenter, allParamsPlot, dataRate, timeShift, duration, regCheck, numPhases, varargin)
% This function plots the increase that occurs from one bar being presented
% to two bars being presented (i.e. from when apparent motion onsets) and
% plots it as an averaged point over space and time
...
secondBarDelay = paramsPlot.secondBarDelay + calciumResponseTime;
presRate = 60; % Hz
...
pointsForSubtracting = find(tVals>=firstBarDelay & tVals<=secondBarDelay);
...
for i = 1:length(polDirBlockStart)
    phaseCenter = 4;
    respsOfInt = bsxfun(@minus, roiSummaryMatrix(phaseCenter+(i-1)*numPhases, pointsOfInterestForAveraging, :), mean(roiSummaryMatrix(phaseCenter+(i-1)*numPhases, pointsForSubtracting, :)));
...
%     legend({'T4 Prog',  'T4 Reg', 'T5 Prog', 'T5 Reg'});
    ylabel('Average \Delta \Delta F/F');
    title(sprintf('Average \Delta \Delta F/F from %0.2fs to %ds', secondBarDelay, duration));
    axis tight;
