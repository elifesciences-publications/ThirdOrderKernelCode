function FitPowerMap(tw,varargin)
    ...
    fitMat = zeros(length(powerMap{1}(:)),numMaps);
    fitMat(:,1) = 1;
    
    for mm = 1:numMaps-1
        fitMat(:,mm+1) = powerMap{mm}(:);
    end
    
    weights = fitMat\powerMap{end}(:);
    
    combMap = reshape(fitMat*weights,[size(powerMap{1},1) size(powerMap{1},2)]);
