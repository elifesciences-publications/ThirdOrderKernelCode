function newmap = b2r(cmin_input,cmax_input,center_val, color_num)
...
if nargin <2 || nargin >4;
   ...
   disp('or input three variables, where the last is center which is white')
end
...
if (cmin_input < center_val)  &&  (cmax_input > center_val) ;  
    ...
    if abs(cmin_input) < cmax_input 
      ...
      %    squeeze(colormap(round((cmin+cmax)/2/cmax),size(colormap)))
       ...
       start_point = round((cmin_input+cmax_input-2*center_val)/2/(cmax_input-center_val)*color_num);
       if start_point == 0
           start_point = 1;
       end
