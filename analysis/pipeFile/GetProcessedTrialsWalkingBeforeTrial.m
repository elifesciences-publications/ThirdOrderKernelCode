function analysis = GetProcessedTrialsWalkingBeforeTrial(flyResp,epochs,params,dataRate,dataType,varargin)
    ...
    %% check that the fly was walking prior to a given trial
    aveWalkingBeforeEpoch = ReduceDimension(walkRespDuringInterleave,'time');
    isWalkingBeforeEpoch = cellfun(@(x)(abs(x(:,:,2))>walkingCutoff),aveWalkingBeforeEpoch,'UniformOutput',false);
    
    for ee = 1:size(normalizedWalking,1)
        for rr = 1:size(normalizedWalking,2)
            activeTrials{ee,rr} = normalizedWalking{ee,rr}(:,isWalkingBeforeEpoch{ee},:);
        end
    end
    ...
    if meanSubtract
        ...
        meanSubtracted = MeanSubtractTurns(activeTrials,turnMeans);
    else
        meanSubtracted = activeTrials;
    end
