 function analysis = SweepTwoPhotonAnalysisDtBar(flyResp,epochs,params,~,dataRate, dataType,interleaveEpoch,varargin)
...
%     finalTitle = [fTitle ': ' epochsForSelectivity{1} ' - ' epochsForSelectivity{2}];
    ...
    plotFigureSem = findobj('Type', 'Figure', 'Name', [figureName ' dt=1 Bars']);
    ...
    if ~isempty(flyEyes)
        ...
        barSubplotTop = subplot(2, 1,1);
        barSubplotBottom = subplot(2, 1,2);
        if newPlot || (length(barSubplotTop.findobj('Type', 'Bar'))==1 && plotOrderChange)%We're in the progressive regime now
            finalTitle = ['Progressive ROIs ' num2str(numROIs(1)) sprintf(',%d', numROIs(2:end))];
            ...
            if ~isempty(prefEpochs)% && ~plotOrderChange
                ...
                % prefEpochs(2) is dt=1 (dt=0 is at 1)
                dataToPlot = [respMatPlot([prefEpochs(2) nullEpochs(2) prefEpochs(end)])' respMatDiffPlot(prefEpochs(2))];
                errorToPlot = [respMatSemPlot([prefEpochs(2) nullEpochs(2) prefEpochs(end)])' respMatDiffSemPlot(prefEpochs(2))];
                PlotXvsY([1 4 7 9], dataToPlot, 'error', errorToPlot, 'graphType', 'bar');
            end
            ...
            if ~isempty(prefEpochs)% && plotOrderChange
                ...
                % prefEpochs(2) is dt=1 (dt=0 is at 1); uncorrelated epochs
                % should be the same(ish)
                dataToPlot = [respMatPlot([prefEpochs(2) nullEpochs(2) prefEpochs(end)])' respMatDiffPlot(prefEpochs(2))];
                errorToPlot = [respMatSemPlot([prefEpochs(2) nullEpochs(2) prefEpochs(end)])' respMatDiffSemPlot(prefEpochs(2))];
                PlotXvsY([2 5 7 10], dataToPlot, 'error', errorToPlot, 'graphType', 'bar');
            end
            
            % We're finding the average uncorrelated response between
            % positive and negative correlations, then replotting it with
            % the appropriate SEM
            if length(barSubplotTop.findobj('Type', 'Bar'))>1
                errorBars = barSubplotTop.findobj('Type', 'ErrorBar');
                resps = cat(1, errorBars.YData);
                sems = cat(1, errorBars.LData);
                uncorrResps = resps(:, 3);
                uncorrSEMs = sems(:, 3);
                
                avgResp = mean(uncorrResps);
                avgSEM = sqrt(mean(uncorrSEMs.^2))
                PlotXvsY(7, avgResp, 'error', avgSEM, 'graphType', 'bar');
            end
            
            title(finalTitle);
            plotFigureSem.Name = [figureName ' dt=1 Bars'];
        else % Regressive regime
            finalTitle = ['Regressive ROIs ' num2str(numROIs(1)) sprintf(',%d', numROIs(2:end))];
            ...
            % positive and negative correlations, then replotting it with
            % the appropriate SEM
            if length(barSubplotBottom.findobj('Type', 'Bar'))>1
                errorBars = barSubplotBottom.findobj('Type', 'ErrorBar');
                resps = cat(1, errorBars.YData);
                sems = cat(1, errorBars.LData);
                uncorrResps = resps(:, 3);
                uncorrSEMs = sems(:, 3);
...
%                 respsBottomCombo([1 4 7 9]) = resps(:, 2);
%                 respsBottomCombo([2 5 7 9]) = resps(:, 1);
%                 respsBottomCombo(7) = avgResp;
%                 
%                 semsBottomCombo([1 4 7 9]) = sems(:, 2);
%                 semsBottomCombo([2 5 7 9]) = sems(:, 1);
%                 semsBottomCombo(7) = avgSEM;
%                 normalizedResp = 
                PlotXvsY(7, avgResp, 'error', avgSEM, 'graphType', 'bar');
            end
            ...
            flyResponseBars = flySemAxes.findobj('type', 'Bar');
            [flyResponseBars.BarWidth] = deal(0.3);
            
            if length(flyResponseBars)>2
