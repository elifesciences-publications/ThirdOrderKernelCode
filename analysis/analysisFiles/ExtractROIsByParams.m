function [roiIndsOfInterest, pValsSum, valueMatrix] = ExtractROIsByParams(Z)
...
% end
roiIndsOfInterest = logical(zeros(1, size(roiAvgIntensityFilteredNormalized, 2)));
...
%     end
    ...
    valueMatrix = [dirSel; dprimish; meanValsSB; pVals; diffHzPower; meanValsCB; permute(roiSize(1:length(meanValsSB)), [2 3 1])];

    roiIndsOfInterest = dirSel>0.4 & dprimish>1 & meanValsSB > 0;
end
...
% 
if isfield(Z.params, 'roiType') && isfield(Z.ROI, 'typeFlag') && ~isempty(roiIndsOfInterest)
    ...
    roiIndsOfInterest = cellfun(@(splitName) any(ismember(splitNames, splitName)), splitFlagNames)' & roiIndsOfInterest;
    roiIndsOfInterest = ~cellfun('isempty', strfind(Z.ROI.typeFlagName, Z.params.roiType)) & roiIndsOfInterest';
