function analysis = BarPairNewCompiledRoiAnalysis(flyResp,epochs,params,~ ,dataRate,dataType,interleaveEpoch,varargin)
...
%             paramsEdge = params{ff}(strcmpi(epochNames, epochsForSelection{ff}{1}));
%             edgeVel = abs(paramsEdge.velocity);
        else
            roiResponsesOut = flyResp{ff};
            epochsForRois = epochs{ff};
...
%         regCheck = 'We don''t use this anymore...';
        ...
        if false && ~simulation && size(epochsForSelectionForFly, 2) > 2
            ...
            % selection is {pref edge pol, pref edge non-pol, null edge pol, null edge non-pol}
            ...
            for roiNum = 1:numROIs(ff)
                ...
                [roiTrialResponseMatrixSem, matDescriptionSem] = ComputeBarPairResponseMatrix(averagedTrialsSEM(interleaveEpoch+1:end, roiNum), barPairSortingStructure, barToCenter, mirrorCheck, optimalBar, numPhases);
                
               
                
                if ignoreNeighboringBars % This is to combine older data, which has no neighboring responses, with the newer data, so we can look at just the directional responses
                ...
                if plotIndividualROIs
                    ...
                    if calculateModelMatrix
                        ...
                        [figureHandles{end+1} allAxesHandles{end+1}] = BarPairPlotFunction(apparentMinusSingleResponse(:, :, end), modelMatrixOneRoi, barToCenter, paramsPlot, dataRate, snipShift/1000, duration/1000, regCheck, numPhases, 'PlotType', 'Real-LinModel');
                        fullText = sprintf('%s\n Fly %d - ROI %d/%d\n Real - Model', textDirection, ff,  roiNum, numROIs(ff));
                        text(0, 0, fullText, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'middle');
                ...
                roiTrialSubOppMatrixes(:, :, roiNum) = roiTrialSubOppMatrix;
                if calculateModelMatrix 
                    roiModelMatrixes(:, :, roiNum) = modelMatrixOneRoi;
                    roiModelSingleMatrixes(:, :, roiNum) = modelSingleResponseOneRoi;
                end
                if calculateModelMatrix  && ~isempty(neighBarPPDescStart) % Check if there are, in fact, next nearest bars
            ...
            if plotIndividualFlies
                [figureHandles{end+1}, allAxesHandles{end+1}] = BarPairPlotFunction(roiTrialResponseMatrixes, flyResponseMatrixSem(:, :, ff), barToCenter, paramsPlot, dataRate, snipShift/1000, duration/1000, regCheck, numPhases, 'PlotType', 'Real');
                ...
                if calculateModelMatrix && ~isempty(neighBarPPDescStart) 
                    [figureHandles{end+1}, allAxesHandles{end+1}] = BarPairPlotFunction(roiModelNeighMatrixes, flyResponseMatrix(:, :, ff), barToCenter, paramsPlot, dataRate, snipShift/1000, duration/1000, regCheck, numPhases, 'PlotType', 'NeighModel');
                    fullText = sprintf('%s\n Fly %d - all %d ROIs\n neigh', textDirection, ff,  numROIs(ff));
                    text(0, 0, fullText, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'middle');
                    figNames = strcat({figureHandles{end}.Name}, 'NModel ');
                    [figureHandles{end}.Name] = deal(figNames{:});
                    
                    [figureHandles{end+1}, allAxesHandles{end+1}] = BarPairPlotFunction(roiRealMinusNeighMatrixes, modelMatrixNeigh, barToCenter, paramsPlot, dataRate, snipShift/1000, duration/1000, regCheck, numPhases, 'PlotType', 'Real-NeighModel');
    ...
    flyRespSubOppMatrix(:, :, all(all(flyRespSubOppMatrix == 0))) = [];
    if plotFlyAverage
        [figureHandles{end+1}, allAxesHandles{end+1}] = BarPairPlotFunction(flyResponseMatrix, NanSem(flyResponseMatrix, 3), barToCenter, paramsPlot, dataRate, snipShift/1000, duration/1000, regCheck, numPhases, 'PlotType', 'Real');
        ...
        [figureHandles{end+1}, allAxesHandles{end+1}] = BarPairPlotFunction(flyRespSubOppMatrix, flyRespSubOppMatrix, barToCenter, paramsPlot, dataRate, snipShift/1000, duration/1000, regCheck, numPhases, 'PlotType', 'SubOpp');
        fullText = sprintf('%s\n All flies (%d)\n SubOpp', textDirection, numFlies);
        text(0, 0, fullText, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'middle');
        figNames = strcat({figureHandles{end}.Name}, 'SubOpp ');
        [figureHandles{end}.Name] = deal(figNames{:});
        
        if calculateModelMatrix
            % Linear model
            flyModelMatrix(:, :, all(all(flyModelMatrix==0))) = [];
            [figureHandles{end+1}, allAxesHandles{end+1}] = BarPairPlotFunction(flyModelMatrix, nanmean(flyResponseMatrix, 3), barToCenter, paramsPlot, dataRate, snipShift/1000, duration/1000, regCheck, numPhases, 'PlotType', 'LinModel');
            ...
            [figureHandles{end+1}, allAxesHandles{end+1}] = BarPairPlotFunction(flyRealMinusModelMatrix, modelMatrixOneRoi, barToCenter, paramsPlot, dataRate, snipShift/1000, duration/1000, regCheck, numPhases, 'PlotType', 'Real-LinModel');
            ...
            if ~isempty(neighBarPPDescStart)
                ...
                [figureHandles{end+1}, allAxesHandles{end+1}] = BarPairPlotFunction(flyModelNeighMatrix, nanmean(flyResponseMatrix, 3), barToCenter, paramsPlot, dataRate, snipShift/1000, duration/1000, regCheck, numPhases, 'PlotType', 'NeighModel');
                ...
                [figureHandles{end+1}, allAxesHandles{end+1}] = BarPairPlotFunction(flyRealMinusNeighMatrix, modelMatrixNeigh, barToCenter, paramsPlot, dataRate, snipShift/1000, duration/1000, regCheck, numPhases, 'PlotType', 'Real-NeighModel');
...
analysis.subOppResps = roiTrialSubOppMatrixes;
