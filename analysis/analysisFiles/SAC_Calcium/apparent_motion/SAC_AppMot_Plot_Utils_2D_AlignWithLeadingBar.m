function SAC_AppMot_Plot_Utils_2D_AlignWithLeadingBar(resp, data_info, stim_info, trial_ID)
...
[resp_over_leadCont, data_info_over_leadCont, ~] = SAC_AverageResponse_By(resp, data_info, 'leadCont','sub',trial_ID);
epoch_index_over_leadCont = data_info_over_leadCont.epoch_index;
[~, resp_over_leadCont_ave, resp_over_leadCont_sem] = SAC_GetAverageResponse(resp_over_leadCont);
...
for ll = 1:1:n_lag
    ...
    resp_ave_this = resp_over_leadCont_ave(:, epoch_index_over_leadCont(ll));
    resp_sem_this = resp_over_leadCont_sem(:, epoch_index_over_leadCont(ll));
    ...
    for cc_lead = 1:1:2 %  Lef and Right Pannel.
        trial_this = trial_ID(epoch_index(cc_lead,ll),:); % trial_this has two dimensions.
        trial_this_reshape = reshape(trial_this, 2, 2);
        for dd = 1:1:n_dir
            for cc_lag = 1:1:2
                block_idx_mega = (block_idx - 1) * 2 + cc_lead;
                block_idx_tmp = (block_idx_mega - 1) * 2 + dd;
                ...
                %% trial this would be two dimensional.
                imagesc(stim_cont(:,:,abs(trial_this_reshape(cc_lag, dd)))'); colormap(gray);
                set(gca, 'XTick',[], 'YTick', [], 'clim',[-1,1],'YDir','normal');
                ConfAxis('fontSize',10);
                set(gca, 'XColor', color_bank(cc_lead,:),'YColor', color_bank(cc_lead,:));
                box on
                ylabel('space','Color', [0,0,0]);
                xlabel('time', 'Color', [0,0,0]);
                title(num2str(trial_this_reshape(cc_lag, dd)));
            end
        end
        
        % plot response of individual traces.
        subplot(n_lag * 3, 1,  subplot_idx_two_resp);
        resp_this_mean = resp_ave(:, epoch_index(cc_lead,ll));
        resp_this_std = resp_sem(:, epoch_index(cc_lead,ll));
        
        PlotXY_Juyue(resp_time,resp_this_mean,'errorBarFlag',1,'sem',resp_this_std,...
            'colorMean', color_bank(cc_lead,:), 'colorError', color_bank(cc_lead,:));
        plot_utils_shade([stim_onset,stim_onset + stim_time], [-0.1, 0.6]); hold on;
        plot([1.25, 1.25], get(gca, 'YLim'), 'k--'); % onset of the second bar.
        set(gca,'YLim', [-0.15,0.05]);
        ylabel('\Delta F/F');
        hold on;
