function SAC_Scintillator_Utils_3D_Dt_Pol_Dir(resp, data_info,stim_info, epoch_ID)
...
for tt = 1:1:n_time
    for pp = 1:1:n_par
        subplot_idx_two_resp  = (pp - 1)*18 + 6 + tt;
        subplot_idx_diff_resp = (pp - 1)*18 + 12 + tt;
        % get the direction subtracted resposne.
        epoch_over_X = epoch_index_over_X(tt, pp);
...
%             SAC_SineWave_Plot_Utils_PlotTimeTraces(resp_ave_this, resp_sem_this, color_use);
            SAC_Scintillator20190326_Plot_Utils_PlotTimeTraces(resp_ave_this, resp_sem_this, color_use);
%             set(gca, 'YLim', [-0.05,0.25]);
            
            %% plot stimulus
            subplot_idx_stim = (pp - 1)*36 + (tt - 1) * 2  + dd;
