function [ resp ] = flyResp( whichOrder,filters,maxTau,x,y,noiseVar,XY )
...
if whichOrder(1)
    ...
    if XY(2)
        resp = resp + filter(filters{1},1,y);
    end
end
...
if whichOrder(2)
    ...
    for r = 1:(dur)-(maxTau-1)
        thisResp(r+(maxTau-1),1) = flipud(x(r:r+maxTau-1))'*filters{2}*flipud(y(r:r+maxTau-1));
    end
    resp = resp + thisResp;
end

if whichOrder(3)
    thisResp = specialthreedfilt(maxTau,x,x,y,filters{3}(:))';
