...
void mexFunction(	
	...
	for(int tau1=0; tau1 < mxl; tau1++){
        ...
        for(int t=mxl-1; t<dataDims; t++){
            stored1a[t] = input1[t-tau1];
            stored1b[t] = input2[t-tau1];
            stored1c[t] = input3[t-tau1];
        }
		for(int tau2=0; tau2 < mxl; tau2++){
            // store resp(t) * input1(t+tau1) * input2(t+tau2) vector
            for(int t=mxl-1; t<dataDims; t++){
                stored2a[t] = stored1a[t] * input2[t-tau2];
                stored2b[t] = stored1b[t] * input3[t-tau2];
                stored2c[t] = stored1c[t] * input1[t-tau2];
            }
                // use stored2 for all tau3 values
            for(int tau3=0; tau3 < mxl; tau3++){
                for(int t=mxl-1; t < dataDims; t++){
                    kernel[tau1+mxl*tau2+mxl*mxl*tau3] += resp[t] * ( 
                            stored2a[t]*input3[t-tau3]* + 
                            stored2b[t]*input3[t-tau3]* + 
                            stored2c[t]*input3[t-tau3]
                            );
                }
            }
        }
