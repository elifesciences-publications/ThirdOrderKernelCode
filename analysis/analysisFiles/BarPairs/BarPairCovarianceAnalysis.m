function [covarStruct, respStruct] = BarPairCovarianceAnalysis(barPairResponsesMatrix, barPairModelResponseMatrix, params, optimalResponseField, snipShift, duration, barToCenter, numPhases, numROIs, figureNamePrepend, plotPlots)
...
responseCovariances = cov(timeAvgResponses');
covarStruct.meanSecondBarPres = responseCovariances;
respStruct.meanSecondBarPres = timeAvgResponses';
if plotPlots
    covarFig = BarPairPlotResponseCovariance(responseCovariances, barToCenter, numPhases, optimalResponseField, 2);
    ...
    covarFig = BarPairPlotResponseCovariance(responseCovariances, barToCenter, numPhases, optimalResponseField, 1);
    ...
    covarFig = BarPairPlotResponseCovariance(responseCovariances, barToCenter, numPhases, optimalResponseField, 0);
...
%% Concatenate full time trace responses vertically, then do covar of them
perFlyAvg = permute(barPairResponsesMatrix(:, tVals>0 & tVals<bothBarsOff, :), [3 2 1]);
allTTDownRows = reshape(perFlyAvg, size(perFlyAvg, 1)*size(perFlyAvg, 2), size(perFlyAvg, 3));
alphaValue = .05;%1-2*normcdf(-1);% alpha of 1 std
responseCovariances = cov(allTTDownRows);
covarStruct.firstBarPres = responseCovariances;
respStruct.firstBarPres = allTTDownRows;
if plotPlots
    covarFig = BarPairPlotResponseCovariance(responseCovariances, barToCenter, numPhases, optimalResponseField, 2);
    title('Covar full time trace all vs all');
    covarFig.Name = [figureNamePrepend 'Covar full time trace all vs all'];
    
    covarFig = BarPairPlotResponseCovariance(responseCovariances, barToCenter, numPhases, optimalResponseField, 1);
    title('Covar full time trace 4th phase vs all');
    covarFig.Name = [figureNamePrepend 'Covar full time trace 4th phase vs all'];
    
    covarFig = BarPairPlotResponseCovariance(responseCovariances, barToCenter, numPhases, optimalResponseField, 0);
    title('Covar full time trace 4th phase vs 4th phase');
    covarFig.Name = [figureNamePrepend 'Covar full time trace 4th phase vs 4th phase'];
end
...
responseCovariances = cov(allTTDownRows);
covarStruct.secondBarPres = responseCovariances;
respStruct.secondBarPres = allTTDownRows;
if plotPlots
    covarFig = BarPairPlotResponseCovariance(responseCovariances, barToCenter, numPhases, optimalResponseField, 2);
    title('Corrcoef second bar time trace all vs all');
    covarFig.Name = [figureNamePrepend 'Covar second bar time trace all vs all'];
    
    covarFig = BarPairPlotResponseCovariance(responseCovariances, barToCenter, numPhases, optimalResponseField, 1);
    title('Corrcoef second bar time trace 4th phase vs all');
    covarFig.Name = [figureNamePrepend 'Covar second bar time trace 4th phase vs all'];
    
    covarFig = BarPairPlotResponseCovariance(responseCovariances, barToCenter, numPhases, optimalResponseField, 0);
    title('Corrcoef second bar time trace 4th phase vs 4th phase');
    covarFig.Name = [figureNamePrepend 'Covar second bar time trace 4th phase vs 4th phase'];
...
%     end
alphaValue = 1-2*normcdf(-1);% alpha of 1 std
responseCovariances = cov(timeTraceDiffResponses');
covarStruct.secondBarMinusMeanFirstBarPres = responseCovariances;
respStruct.secondBarMinusMeanFirstBarPres = timeTraceDiffResponses';
if plotPlots
    covarFig = BarPairPlotResponseCovariance(responseCovariances, barToCenter, numPhases, optimalResponseField, 1);
...
responseCovariances = cov(timeTraceDiffResponses');
covarStruct.secondBarMinusMeanPrefSingleBarPres = responseCovariances;
respStruct.secondBarMinusMeanPrefSingleBarPres = timeTraceDiffResponses';
if plotPlots
    covarFig = BarPairPlotResponseCovariance(responseCovariances, barToCenter, numPhases, optimalResponseField, 0);
...
if ~isempty(barPairModelResponseMatrix)
    ...
    responseCovariances = cov(modelRealDiffs');
    covarStruct.realMinusModelSecondBarPres = responseCovariances;
    respStruct.realMinusModelSecondBarPres = modelRealDiffs';
    if plotPlots
        covarFig = BarPairPlotResponseCovariance(responseCovariances, barToCenter, numPhases, optimalResponseField, 2);
