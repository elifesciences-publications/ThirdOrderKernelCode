 function analysis = CombAndSeBaraTe(flyResp,epochs,params,stim,dataRate,dataType,interleaveEpoch,varargin)
...
%         end
    ...
    % run the algorithm for each fly
     for ff = 1:numFlies
        ...
        if ~isempty(kill)
            ignoreEpochs = analysis.indFly{ff}{end}.snipMat(numIgnore+1:end-kill,:);       
            %respMatSemPlot = [respMatSemPlot(~isnan(respMatSemPlot(:,2)),1), respMatSemPlot(~isnan(respMatSemPlot(:,2)),2)];
        else
...
%     if ~isempty(kill)
%         for i = 1:length(kill)
%             respMatPlot(kill(i),:) = nan;
%             respMatSemPlot(kill(i),:) = nan;
%         end
%         respMatPlot = [respMatPlot(~isnan(respMatPlot(:,2)),1), respMatPlot(~isnan(respMatPlot(:,2)),2)];
%         respMatSemPlot = [respMatSemPlot(~isnan(respMatSemPlot(:,2)),1), respMatSemPlot(~isnan(respMatSemPlot(:,2)),2)];
%     end
