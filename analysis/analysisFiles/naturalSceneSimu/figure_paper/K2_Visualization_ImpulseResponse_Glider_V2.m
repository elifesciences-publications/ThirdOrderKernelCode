function K2_impulse = K2_Visualization_ImpulseResponse_Glider_V2(K2,K2_individual, K2_noise, varargin)
...
n_fly = size(K2_individual, 2);
K2_individual_glider = zeros([size(K2_glider, 1), n_fly]);
for ff = 1:1:n_fly
...
% K2_individual_glider_mean = mean(K2_individual_glider, 2);
K2_individual_glider_std = std(K2_individual_glider, 1,2);
...
if plot_flag
    ...
    x_tick_str = strsplit(num2str(x_tick_str_num,2));
    y_tick = [0,16,31,46];
    ...
    for ii = 1:1:1
        ...
        quickViewOneKernel(K2_impulse(1:tMaxShow, :), 1, 'labelFlag', false, 'set_clim_flag', true, 'clim', maxValue);
        colorbar('off');
        if ii == 3
            colorbar
            c = colorbar;
    ...
    for ii = 1:1:1
        axes('Units', h(2, 1).Units,'Position', h(2, 1).Position);
        ...
        set(gca, 'YLim',[-maxVal, maxVal],'XLim',[1 - 0.5,length(dtxy_bank) + 0.5]);
        
        
    end
    ylim = get(gca, 'YLim');
    x1 = xlabel('\tau2 - \tau1 [ms]');
    x1.Position = [tau1_tau2_middle, ylim(1) * 1.5];
    set(gca, 'XTick', []);
    for xx = 1:1:length(dtxy_bank)
        text(xx, ylim(1),  x_tick_str{xx},'Rotation',90,'HorizontalAlignment','center');
    end
    High_Corr_PaperFig_Utils_SmallFontSize;
end
end
