function Generate_VisStim_VelEst_OneRowAllPhaseAllVel_RealKernel(image_process_info,image,stim,velocity,time, kernel, vel_range_bank, varargin)
%% get third order kernel in...
...
for m = 1:1:nImage % 5 rows. not sure.
    ...
    for k = 1:1:nSpI(imageID);
        ...
        parfor vv = 1:1:length(vel_range_bank)
            ...
            v_est_ns_all = VelocityEstimation_OneStim_InputIsOneRow(stim_ns_full, kernel);
            v_est_sc_all =  VelocityEstimation_OneStim_InputIsOneRow(stim_sc_full, kernel);
            
            v2_ns(:,vv,sample_counter) = v_est_ns_all.v2;
            v2_sc(:,vv,sample_counter) = v_est_sc_all.v2;
            v3_ns(:,vv,sample_counter) = v_est_ns_all.v3;
            v3_sc(:,vv,sample_counter) = v_est_sc_all.v3;
...
% store the second and the third. do the analysis and take a look. Whether
% third order kernel can help.
...
v2 = v2_ns; v3 = v3_ns; v_real = v_real_all;
save(storage_full_path_ns, 'v2','v3','v_real'); clear v2 v3
...
v2 = v2_sc; v3 = v3_sc;
save(storage_full_path_sc, 'v2','v3','v_real'); clear v2 v3
