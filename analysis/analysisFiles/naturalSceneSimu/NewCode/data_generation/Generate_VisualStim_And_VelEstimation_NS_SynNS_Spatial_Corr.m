function Generate_VisualStim_And_VelEstimation_NS_SynNS_Spatial_Corr(image_process_info,image,stim,velocity,time, kernel, varargin)
...
if synthetic_flag
    synthetic_type = 'maintain_spatial_corr';
    visual_stimulus_relative_path_spatial_corr = NS_Filema_Param_To_FolderName(velocity, image_process_info, [],'folder_use','visual_stimulus','synthetic_flag',synthetic_flag,'synthetic_type',synthetic_type);
    visual_stimulus_full_relative_path_spatial_corr = fullfile(S.natural_scene_simulation_path, 'visual_stimulus', visual_stimulus_relative_path_spatial_corr);
    
    synthetic_type = 'scramble_phase';
...
if synthetic_flag
    ...
    data_storage_unit_spatial_corr = data_storage_struct;
end
for m = 1:1:nImage
    ...
    for k = 1:1:nSpI(imageID);
        ...
        if synthetic_flag
            switch synthetic_type
                case 'scramble_phase'
                    [oneRow_scrambled, oneRow_spatial_corr] = ...
                                        Generate_VisualStim_And_VelEstimation_Utils_Spatial_Corr(oneRow);
                    ...
                    %                     oneRow_spatial_corr = Generate_VisualStim_And_VelEstimation_Utils_Spatial_Corr(oneRow);
                    switch image_process_info.contrast
                        case 'static'
                            stim_full_spatial_corr = VisualStimulusGeneration_Utils_CreateXT(oneRow_spatial_corr, v_real, time, image);
                            stim_spatial_corr = stim_full_spatial_corr(:, column_pos);
                    end
                    v_est_spatial_corr = VelocityEstimation_OneStim(stim_spatial_corr, kernel);
                    
                    data_spatial_corr.image_ID = imageID;
                    data_spatial_corr.flip_flag = flip_flag;
                    data_spatial_corr.row_pos = row_pos;
                    data_spatial_corr.stim = stim_spatial_corr;
                    data_spatial_corr.v_real = v_real;
                    data_spatial_corr.column_pos = column_pos;
                    data_spatial_corr.v_est = v_est_spatial_corr;
                    data_storage_unit_spatial_corr(storage_sample_unit_counter) = data_spatial_corr;
            ...
            if synthetic_flag
                switch synthetic_type
                    case 'scramble_phase'
                        ...
                        data_storage_unit_sc =  data_storage_struct; % clear the old, start a new one
                        
                        storage_full_path_spatial_corr = fullfile(visual_stimulus_full_relative_path_spatial_corr, unit_name);
                        if ~exist(visual_stimulus_full_relative_path_spatial_corr, 'dir')
                            mkdir(visual_stimulus_full_relative_path_spatial_corr)
                        end
                        save(storage_full_path_spatial_corr, 'data_storage_unit_spatial_corr');
                        data_storage_unit_spatial_corr =  data_storage_struct; % clear the old, start a new one
