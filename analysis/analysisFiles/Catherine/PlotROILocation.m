 function analysis = PlotROILocation(flyResp,epochs,params,stim,dataRate,dataType,interleaveEpoch,varargin)
    ...
    % plot for the first 4 epochs
    respMatPlot = respMatPlot(:, 1:4);
    respMatSemPlot = respMatSemPlot(:, 1:4);
    %%
    ...
    for pp = 1:size(respMatPlot,3)
        ...
        MakeFigure;
        PlotXvsY(degX,respMatPlot(:,:,pp),'error',respMatSemPlot(:,:,pp));
        hold on;
        PlotConstLine(0);
        PlotConstLine(0,2);
...
%         PlotConstLine(5000,2);
        ...
        ConfAxis('tickX',plotDeg,'tickLabelX',plotTimeLabel,'labelX','location (degrees)','labelY',[yAxis{pp} ' - ' num2str(numFlies) ' flies'],'fTitle', finalTitle,'figLeg',figLeg);
        hold off;
    end
catch err
    analysis = {};
