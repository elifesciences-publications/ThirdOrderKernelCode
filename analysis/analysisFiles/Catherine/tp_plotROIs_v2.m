function tp_plotROIs_v2(filteredMovie, roiMasks, roiCOM)
...
colors = jet(size(roiMasks, 3));
for i = 1:size(roiMasks, 3)
    roiMask = logical(roiMasks(:, :, i));
...
if isempty(roiColor)
    for i = 1:size(roiMasks, 3)
        ...
        text(roiCOM(i, 2), roiCOM(i, 1), num2str(i), 'HorizontalAlignment', 'center', 'Color',  color);
    end
end
