...
for pp = 1:1:n_pair
    ...
    plot(trace_2);
    hold off
    legend(['ESI',num2str(ESI_1), '  DSI', num2str(DSI_1)],...
           ['ESI',num2str(ESI_2), '  DSI', num2str(DSI_2)])
    subplot(2,1,2);
    plot(trace_c);
    legend(['ESI',num2str(ESI_c), '  DSI', num2str(DSI_c)]);
    title(num2str( ESI_DSI_NONDEC(pp,:)));
