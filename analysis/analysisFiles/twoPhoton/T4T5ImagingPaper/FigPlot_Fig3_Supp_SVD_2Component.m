function FigPlot_Fig3_Supp_SVD_2Component(roiData,folderStr,saveFigFlag,varargin)
smoothFlag = false;
figFileType = {'fig'};
nFigSave = 1;
nType = 4;
...
%     subplot(4,4,8 * (ii - 1) + 2)
    ...
    title('kernel - (1st + 2nd) component');
    ...
    subplot(4,4,8 * (ii - 1) + 2);
    quickViewOneKernel_Smooth(U(:,2)*diagS(2)*V(:,2)' + U(:,1)*diagS(1)*V(:,1)',1,'colorbarFlag',false,'labelFlag',false);
    title('1 + 2 component');
end
if saveFigFlag
    MySaveFig_Juyue(gcf,'1o_Kernel_SVDT4T5_2Component','' ,'nFigSave',nFigSave,'fileType',figFileType);
end
%
% typeStr = {'T4 Pro','T4 Reg','T5 Pro','T5 Reg'};
% for tt = 1:1:4
