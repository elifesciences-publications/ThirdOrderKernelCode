function alignRespStim = tp_AlignStimRespforKernelInPhotoDiode(Z,epochForKernel,epochForKernelFlag,nanCullFlag,interpolateRespFlag)
...
if  ~isempty(find(nanStimulus,1))
    ...
    % you have to check that whether it is only in the middle, or
    % otherwise, it is very dangerou to just ignore them.
    % set a debug point, where the nan appears in the middle of the
    % array...
    if find(nanStimulus,1,'last') ~= nT
        warning('stimulusDataInds nan value appears in the middle of the array')
    end
...
% where this nan comes from? nan, but not movement....
if nanCullFlag
    for rr = 1:1:nRoi
...
if epochForKernelFlag
    ...
    for rr = 1:1:nRoi
        ...
        indForKernelEpoch = epochOfStimulusRoi == epochForKernel;
        endFramesProbEpoch = find(epochOfStimulusRoi == epochForKernel - 1,1,'last');
        
        % if the last probe stimulus happens before the multibar stimulus,
        % then, there is no last probe stimulus.
        if endFramesProbEpoch < find( indForKernelEpoch,1,'first')
             kernelInd =  indForKernelEpoch;
        else
            indBeforeLastEpoch = (1:length(epochOfStimulusRoi))' < endFramesProbEpoch;
            if sum(~indBeforeLastEpoch & indForKernelEpoch)
                disp(rr)
            end
            kernelInd = indBeforeLastEpoch & indForKernelEpoch;
        end
        responseDataKernel{rr} = responseData{rr}(kernelInd);
        stimulusIndexesForRoiKernel{rr} =  stimulusIndexesForRoi{rr}(kernelInd);
        % ATTENTION. There is epoch 13 after the last probe stimulus.
    end
end

if interpolateRespFlag
    [responseDataKernel,stimulusIndexesForRoiKernel] = ...
        cellfun(@(resp,stimIndex) tp_AlignStimRespforKernelInPhotoDiode_Utils_Interpolation(resp,stimIndex)...
        ,responseDataKernel,stimulusIndexesForRoiKernel,'UniformOutput',false);
end
