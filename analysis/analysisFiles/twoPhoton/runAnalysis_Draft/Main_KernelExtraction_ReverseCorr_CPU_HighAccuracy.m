function kernels = Main_KernelExtraction_ReverseCorr_CPU_HighAccuracy(respData, stimData, stimIndexes, varargin)
order = 1;
maxTau = 64;
dxBank_for_third_kernel = {[0,1],[0,-1],[1,2],[-1,-2]}; 
arma_flag = false;
kr = [];
donoise = false;
...
%             sumGPU = oned_gpu(maxTau,stimDataGPU1,respDataGPU);
            % compare this with first order cpu.
        ...
        case 3
            % actually,  you will loop through dxBank here...
             kernels_this_rep = cell(length(dxBank_for_third_kernel),1);
            for dxx = 1:1:length(dxBank_for_third_kernel)
                dx = dxBank_for_third_kernel{dxx};
                ...
                % work on third order kernel extraction.
...
%                 fprintf('It takes %f seconds to organize the kernel\n',tOragnize);
                ...
                kernels_this_rep{dxx} =  kernels_this_rep_this_dx;
            end
    end
