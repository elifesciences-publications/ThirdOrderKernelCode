function roiDataAna = roiAnalysis_AllRoi_analyzeRepSegAndModelPred_OLS(roiData,varargin)
nRoi = length(roiData);
...
for rr = 1:1:nRoi
    ...
    roi = roiAnalysis_OneRoi_OLS_LN_And_Second(roi,param,'order',1);
    toc
    tic
    maxTauRange = [20];
    dtMaxRange = [8];
    ...
    param.barNumRange = barNumRange;
    secondParam = param;
    roi = roiAnalysis_OneRoi_OLS_LN_And_Second(roi,param,'order',2);
    toc
    roi = roiAnalysis_OneRoi_OLS_R_1o2o_Combine(roi); % have not change it yet...
