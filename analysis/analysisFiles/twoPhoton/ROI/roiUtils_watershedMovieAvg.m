function sheds = roiUtils_watershedMovieAvg( Z, fgThresh )
...
% the nearest watershed center of mass.
    ...
    preMeanImg = mean(Z.grab.imgFrames,3); % unfiltered, unwindowed
    ...
    % everything.
    preSeg = double(preSeg) .* Z.grab.windowMask;  
    if watershedDebugMode
        borders = (preSeg == 0);
        figure; 
    ...
    meanImg = meanImg .* Z.grab.windowMask;
    ...
    getBorder = double( preSeg == 0 ) .* Z.grab.windowMask;
