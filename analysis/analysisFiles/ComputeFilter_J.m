function D = ComputeFilter_J(varargin)
    ...
    for jj = 1:numEpochs
        ...
        for ii = 1:analysis.OD.numFlies
            filterCalc{jj}(:,ii) = oneD_filter(vel{jj},respX{jj}(:,ii),filterLength);
        end
        
        analysis.filterAve{jj} = mean(filterCalc{jj},2);
        analysis.filterSTD{jj} = std(filterCalc{jj},0,2);
        analysis.filterSEM{jj} = analysis.filterSTD{jj}/analysis.OD.numFlies;
    end
...
%     end
    for jj = 1:numEpochs
        figure; plot(linspace(0,filterLength-1,filterLength),analysis.filterAve{jj});
    end
%     
end