function [roiIndsOfInterest, pValsSum, valueMatrix] = ExtractROIsTest(Z)
...
if ~isnumeric(epochsForSelectivity)
    if iscell(epochsForSelectivity)
        epochNum = [];
        for i = 1:2 % We set to two because you only compare two at a time!
...
%     for ind = 1:min([size(selectDataBounds, 2), size(compareDataBounds, 2)]);
        ...
        [indexesSelect, indexesSelectCell ]= IndexesFromBounds(selectDataBounds);
        [indexesCompare, indexesCompareCell ] = IndexesFromBounds(compareDataBounds);
        diffSelectFromBase = roiAvgIntensityFilteredNormalized(indexesSelect, :);%findDiffFromBase(roiAvgIntensityFilteredNormalized, selectDataBounds, triggerInds, epochFractionCompare);
        diffCompareFromBase = roiAvgIntensityFilteredNormalized(indexesCompare, :);%findDiffFromBase(roiAvgIntensityFilteredNormalized, compareDataBounds, triggerInds, epochFractionCompare);
        
        secEpochDSB = find(diff(indexesSelect)~=1)+1;
        secEpochDCB = find(diff(indexesCompare)~=1)+1;
        
        selectAvgPerPres = cellfun(@(x) nanmean(roiAvgIntensityFilteredNormalized(x, :)), indexesSelectCell, 'UniformOutput', false);
        selectAvgPerPres = cat(1, selectAvgPerPres{:});
        compareAvgPerPres = cellfun(@(x) nanmean(roiAvgIntensityFilteredNormalized(x, :)), indexesCompareCell, 'UniformOutput', false);
        ...
        elseif any(isnan(diffSelectFromBase(:, 1)))
            ...
            tempCB = diffCompareFromBase;
            timeValsKnown = timeVals(~isnan(tempCB(:, 1)));
            tempCB(isnan(tempCB(:, 1)), :) = [];
            diffCompareFromBase = interp1(timeValsKnown, tempCB, timeVals);
            diffCompareFromBase(isnan(diffCompareFromBase(:, 1)), :) = [];
            
        end
        
        covMat = cov([diffSelectFromBase(1:secEpochDSB-1, :) diffSelectFromBase(secEpochDSB:end-1, :) ]);
        vars = diag(covMat);
        rvalue = diag(covMat, size(diffSelectFromBase,2))./sqrt(prod([vars(1:end/2)'; vars(end/2+1:end)']))';
...
%         diffSelectFromBase = exp(diffSelectFromBase);
        ...
        alpha = .05;
        [roiIndsOfInterestTemp, pVals] = ttest2(diffSelectFromBase, diffCompareFromBase, 'tail', 'right', 'alpha', alpha/size(roiAvgIntensityFilteredNormalized, 2), 'Vartype', 'unequal');
...
%     end
    ...
    valueMatrix = [dirSel; dprimish; rvalue'; pVals; meanValsSB; meanValsCB; permute(roiSize(1:length(meanValsSB)), [2 3 1]); diffHzPower];
end
