function D = combineEpochRespSepandDoublesLimitTime(varargin)
    ...
    if(timeRange(2) - timeRange(1))
        D.data.resp = D.data.resp(timeRange(1):timeRange(2),:);
        D.data.stim = D.data.stim(timeRange(1):timeRange(2),:);
    end
    ...
    makeFigure;
    plotXvsY(x,[odd(:,:,:,1)' flipEven*even(:,:,:,1)'],xLabel,yLabel,'error',[semOdd(:,:,:,1)' semEven(:,:,:,1)']);
    confAxis(varargin{:});
